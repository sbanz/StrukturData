...
int fibonacciSearch(Node *node, string x, int n){
    ...
    if (F1 && temp2->data.nama_tim == x) return offset + 1;
    return -1;
}
...
void search(Node **head){
    ...
    int index = 0;
    cout << "\nMasukkan Nama Tim yang dicari : ";
    cin>>searchkey;

    int idx = fibonacciSearch(*head, searchkey, lengthh);
    int nomor = idx + 1;
    if (idx >= 0) {
        while (temp != NULL){
            if (index == idx){
                cout <<"\n=============== Data Ke-"<<nomor<<" ================"<<endl
		 			 <<"Nama TIM : "<<temp->data.nama_tim<<endl
		 			 <<"Kota Asal : "<<temp->data.kota_asal<<endl
		 			 <<"Nama Stadion : "<<temp->data.nama_stadion<<endl
		 			 <<"Nama Suporter : "<<temp->data.suporter<<endl
		 			 <<"Jumlah Pemain : "<<temp->data.jumlah_pemain<<endl;
                break;
            }
            index++;
            temp = temp->next;
